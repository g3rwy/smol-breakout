...
struct Player {
	...
	int rand_off_apilot = (rand() % 60) + 20;
...
public:
	...
	void MoveBall(Ball& b,Level& l,Player& p,const float& delta){
		...
		{
			p.rand_off_apilot = (rand() % 50) + 20; // change random offset from center in case of autopilot
			...
			//if(b.pos.x < p.pos.x + (p.length / 2)) ratio = -ratio;
